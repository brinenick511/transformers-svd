...
from .configuration_svd_llama import SVDLlamaConfig
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, config: SVDLlamaConfig, device=None):
        super().__init__()
...
class SVDLlamaAttention(nn.Module):
    ...
    def __init__(self, config: SVDLlamaConfig, layer_idx:int = -1, ratio:float = 1.0):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: SVDLlamaConfig, layer_idx: int):
        ...
        # else:
        self.self_attn = SVDLlamaAttention(config=config, layer_idx=layer_idx,ratio=config.ratio)
        self.mlp = SVDLlamaMLP(config,config.ratio)
...
@auto_docstring
class SVDLlamaPreTrainedModel(PreTrainedModel):
...
@auto_docstring
class SVDLlamaModel(SVDLlamaPreTrainedModel):
...
@auto_docstring
class SVDLlamaForCausalLM(SVDLlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = SVDLlamaModel(config)
        self.vocab_size = config.vocab_size
