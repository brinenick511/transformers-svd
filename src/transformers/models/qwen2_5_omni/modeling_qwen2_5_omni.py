...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        talker_device = self.talker.device
        if talker_device.type == "meta":
            talker_device = self.thinker.device

        # 2. Generate speech tokens from talker module
        embeds_to_talker = thinker_result.hidden_states[0][0].clone().to(talker_device)
        ...
        thinker_generate_ids = thinker_result.sequences[:, input_ids.size(1) :].to(talker_device)
        thinker_token_embeds = [
            token_hidden_states[0].to(talker_device) for token_hidden_states in processed_thinker_hidden
        ]
        thinker_hidden_states = [
            token_hidden_states[-1].to(talker_device) for token_hidden_states in processed_thinker_hidden
        ]
        ...
        talker_input_text_ids = torch.cat(
            [
                ...
                torch.tensor([[talker_text_bos_token]], dtype=torch.long, device=talker_device),
        ...
        talker_input_ids = torch.cat(
            [
                torch.full_like(input_ids, fill_value=self.talker.codec_mask_token, device=talker_device),
                torch.tensor([[self.talker.codec_pad_token]], dtype=torch.long, device=talker_device),
                torch.tensor([[self.talker.codec_bos_token]], dtype=torch.long, device=talker_device),
            ],
            dim=1,
        )
        ...
        talker_text_bos_embed = thinker_embed_tokens(talker_text_bos_token).to(talker_device)
        ...
        talker_result = self.talker.generate(
            ...
            **{k: (v.to(talker_device) if torch.is_tensor(v) else v) for k, v in talker_kwargs.items()},
        )
